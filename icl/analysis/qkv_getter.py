...
class QKVGetterManger:
    ...
    def register_qkvgetter_to_model(self):
        ...
        if self.model.model_name in ["gpt2-xl", "gpt-j-6b"]:
            for i, layer in enumerate(self.model.model.transformer.h):
                ...
                wrapped_attn = wrap_fn_save_query_key_value(
                    qkvgetter=qkvgetter
                )(layer.attn._attn)
                self.setter_with_restore.set(layer.attn._attn, wrapped_attn)
        else:
            raise NotImplementedError(
                f"{self.model.model_name} not supported yet"
            )
        return qkvgetters
    ...
    def collect_qkv(self):
        ...
        qkv = self.predictor._cal_all_key_and_values_of_class(
            inputs=inputs,
            past_key_values=qkv,
...
def cal_results(
    demonstraions, model, tokenizer, training_args, args, seed, dataset
):
    ...
    demo_dataset, empty_test_dataset = prepare_analysis_dataset(
        demonstraions, args, seed, dataset, tokenizer
    )
