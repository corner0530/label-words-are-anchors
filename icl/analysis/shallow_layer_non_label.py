...
from .attentioner import (
    AttentionAdapterNonLabel,
    GPTJAttentionerManager,
...
def shallow_layer_non_label(args: ShallowNonLabelArgs):
    ...
    if args.model_name in ["gpt-j-6b"]:
        attentionermanger = GPTJAttentionerManager(
            model.model, attention_adapters
        )
    ...
    for seed in args.seeds:
        analysis_dataset, analysis_no_demo_dataset = prepare_analysis_dataset(
            seed
        )
        ...
        if args.mask_layer_pos == "first":
            attentionermanger.set_attentioner_state(
                True, list(range(0, args.mask_layer_num))
            )
        elif args.mask_layer_pos == "last":
            attentionermanger.set_attentioner_state(
                True, list(range(num_layer - args.mask_layer_num, num_layer))
            )
        else:
