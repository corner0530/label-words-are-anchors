...
from ..utils.data_wrapper import tokenize_dataset, wrap_dataset
...
from ..utils.other import dict_to, sample_two_set_with_shot_per_class, set_gpu
...
def train(args: ReweightingArgs) -> None:
    """
    Trains a model using the given arguments.

    Args:
        args (ReweightingArgs): The arguments for training.
    ...
    ys: list[
        tuple[
            torch.Tensor, list[float], list[torch.Tensor], torch.Tensor, float
        ]
    ] = []
    for seed in args.seeds:
        analysis_dataset, train_dataset, demonstration = (
