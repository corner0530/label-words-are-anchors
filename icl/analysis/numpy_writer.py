...
class NumpyWriter:
    def __init__(self, log_dir, mode="w", cover=False):
        ...
        assert not (
            mode == "r" and cover
        ), "mode == 'r' and cover == True not allowed"
        if not cover and mode == "w":
            assert not os.path.exists(
                log_dir
            ), f"log_dir {log_dir} ({os.path.abspath(log_dir)}) already exists"
        if cover:
    ...
    def append_tensor(self, tensor, fp, debug=False):
        tensor_numpy = (
            tensor.numpy() if not isinstance(tensor, np.ndarray) else tensor
        )
        ...
        debug_print(
            f"append_tensor {tensor.shape} {tensor.dtype} {tensor_numpy.shape} {tensor_numpy.dtype}",
            flush=True,
        )

    def read_tensors(self, fp, idxs=None, condition=None):
        assert not (
            idxs is not None and condition is not None
        ), "idxs 和 condition 不能同时设置"
        if idxs is not None:
    ...
    def write(
        self, data, name, global_step=None, multiple_tensors=True, debug=False
    ):
...
class CPUTensorBuffer(list):
    def __init__(
        ...
        if self.allow_auto_write:
            assert (
                self.writer_np is not None
            ), "writer_np must be set when allow_auto_write is True"
            assert (
                self.name is not None
            ), "name must be set when allow_auto_write is True"
