...
def get_gpu_ids(gpu_num=100, gpu_ids=None, min_memory: float = 5000):
    ...
    memory_gpu = [
        int(x.split()[2]) for x in open("./buffer/tmp", "r").readlines()
    ]
    enough_memory_gpu_ids = [
        (idx, _) for idx, _ in enumerate(memory_gpu) if _ > min_memory
    ]
    enough_memory_gpu_ids = sorted(
        enough_memory_gpu_ids, key=lambda x: x[1], reverse=True
    )
    auto_gpu_ids = [
        idx
        for idx, _ in enough_memory_gpu_ids[
            0 : max(auto_num, len(enough_memory_gpu_ids))
        ]
    ]
...
class Combination_iterater:
    def __init__(self, hyperparameter_lists: Dict[str, List[Any]]):
        ...
        self.hyperparameter_list_lengths: List[int] = [
            len(_) for _ in hyperparameter_lists.values()
        ]
    ...
    def get_combination(self, i: int) -> Dict[str, Any]:
        ...
        hyperparameters = {
            k: v[index]
            for (k, v), index in zip(self.hyperparameter_lists.items(), indexs)
        }
...
class Sh_run:
    def __init__(
        self,
        hyperparameter_lists: List[Dict[str, List[Any]]],
        gpu_list: List[int] = None,
        default_sh_file="./gpu_sh.sh",
    ) -> None:
        self.experiment_strs = [_["expr"] for _ in hyperparameter_lists]
        for _ in hyperparameter_lists:
            _.pop("expr")
        self.hyper_combinator_iterators = [
            Combination_iterater(hyperparameter_lists=hyperparameter_list)
            for hyperparameter_list in hyperparameter_lists
        ]
    ...
    def run(self):
        _ = 0
        for expr, hyper_combinator_iterator in zip(
            self.experiment_strs, self.hyper_combinator_iterators
        ):
            ...
            for k in self.gpu_str_dict.keys():
                s += (
                    " ".join(
                        [
                            "nohup sh",
                            self.generate_specific_sh_file_name(k),
                            "&",
                        ]
                    )
                    + "\n"
...
def get_perf_from_perf_dict(
    perf_dict: Dict[str, Any], perf_name: Optional[str] = None
):
    if perf_name is None:
        if "acc" in perf_dict:
            ...
            raise ValueError(
                "perf_name is None, and can't find proper erf_name in perf_dict"
            )
